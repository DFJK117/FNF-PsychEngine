...
class PlayState extends MusicBeatState
{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				if(roundSus > 0)
				{
					for (susNote in 0...roundSus)
					{
						...
						if(DoubaoConfig.isTwoPlayer() && swagNote.isOpponent && ClientPrefs.data.doubaoP1DownScroll) sustainNote.flipY = true;
						...
						// Doubao Engine: sustain placement mirrors its parent note (centered / split / vanilla)
						var sustainAnchor:Float = STRUM_X + 50;
						if (DoubaoConfig.isSoloMulti())
							sustainNote.x += DoubaoConfig.rowStartX(1) - sustainAnchor;
						else if (DoubaoConfig.isTwoPlayer())
							sustainNote.x += (swagNote.isOpponent ? DoubaoConfig.rowStartX(0) : DoubaoConfig.rowStartX(1)) - sustainAnchor;
						else if (sustainNote.mustPress)
				...
				// Doubao Engine: horizontal placement per mode.
				// solo multi-K = one centered row; two-player = Dad left / BF right; vanilla 4K unchanged.
				// Note ctor already adds (STRUM_X + 50), subtract it so the spawn column aligns with the receptor.
				var soloMultiN:Bool = DoubaoConfig.isSoloMulti();
				var twoPN:Bool = DoubaoConfig.isTwoPlayer();
				var noteAnchor:Float = STRUM_X + 50;
				if (soloMultiN)
				{
					swagNote.x += DoubaoConfig.rowStartX(1) - noteAnchor; // centered single row
				}
				else if (twoPN)
				{
					swagNote.x += (isOpponentNote ? DoubaoConfig.rowStartX(0) : DoubaoConfig.rowStartX(1)) - noteAnchor;
				}
				else if (swagNote.mustPress)
				{
					swagNote.x += FlxG.width / 2; // vanilla BF offset
				}
				else if(ClientPrefs.data.middleScroll)
				{
					swagNote.x += 310;
	...
	{
		// Doubao Engine: base X per mode (solo multi-K centered / two-player split / vanilla 4K)
		var twoP:Bool = DoubaoConfig.isTwoPlayer();
		var soloMulti:Bool = DoubaoConfig.isSoloMulti();
		var strumLineX:Float = (ClientPrefs.data.middleScroll && !twoP) ? STRUM_X_MIDDLESCROLL : STRUM_X;
		// Doubao Engine: Player 1 (opponent) can have its own scroll direction, including receptor Y
		var p1Down:Bool = (twoP && player < 1) ? ClientPrefs.data.doubaoP1DownScroll : ClientPrefs.data.downScroll;
		var strumLineY:Float = p1Down ? (FlxG.height - 150) : 50;

		var baseX:Float;
		if (soloMulti)
			baseX = DoubaoConfig.rowStartX(1); // single centered player row (opponent row hidden)
		else if (twoP)
			baseX = DoubaoConfig.rowStartX(player); // split left/right rows
		else
			baseX = strumLineX + 50 + ((FlxG.width / 2) * player); // vanilla 4K solo
		...
		for (i in 0...DoubaoConfig.keyCount)
		{
			...
			if (player < 1)
			{
				if(twoP)
					...
					targetAlpha = 0; // solo multi-K chart has no CPU opponent row
				else if(!ClientPrefs.data.opponentStrums) targetAlpha = 0;
				else if(ClientPrefs.data.middleScroll) targetAlpha = 0.35;
			}

			var babyArrow:StrumNote = new StrumNote(baseX, strumLineY, i, player);
			...
			{
				// vanilla middle-scroll split only for 4K solo
				if(ClientPrefs.data.middleScroll && DoubaoConfig.keyCount == 4 && !twoP)
				{
					babyArrow.x += 310;
					if(i > 1) { //Up and Right
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!controls.controllerMode)
		{
			...
			if(FlxG.keys.checkStatus(eventKey, JUST_PRESSED))
			{
				...
				if(DoubaoConfig.isTwoPlayer())
				{
					// P2 (arrows) controls BF on the right, P1 (WASD) controls opponent Dad on the left
				...
				else if(DoubaoConfig.isSoloMulti())
				{
					// solo multi-K uses the symmetric DJMax-style binds (D F SPACE G K etc.)
					final soloIdx:Int = DoubaoConfig.getSoloIndex(eventKey);
					if(soloIdx >= 0) keyPressed(soloIdx);
				}
				else
	...
	private function onKeyRelease(event:KeyboardEvent):Void
	{
		...
		if(DoubaoConfig.isSoloMulti())
		{
			if(controls.controllerMode) return;
			final soloIdx:Int = DoubaoConfig.getSoloIndex(eventKey);
			if(soloIdx >= 0) keyReleased(soloIdx);
			return;
		}
		...
		if(useFixedKeys)
		{
			...
			for (li in 0...DoubaoConfig.keyCount)
			{
				if(DoubaoConfig.isTwoPlayer())
				{
					...
					holdP1.push(FlxG.keys.anyPressed([DoubaoConfig.P1_KEYS[li]]));
				}
				else
					holdP2.push(FlxG.keys.anyPressed([soloBinds[li]])); // solo multi-K symmetric binds
			}
		}
					...
					if (canHit && n.isSustainNote) {
						...
						if(useFixedKeys)
						{
							if(DoubaoConfig.isTwoPlayer())
								...
								isHeld = holdP2[n.noteData]; // solo multi-K: symmetric binds
						}
						else
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(char != null && (note == null || !note.noMissAnimation) && char.hasMissAnimations)
		{
			...
			var animToPlay:String = singAnimations[DoubaoConfig.directionOf(Std.int(Math.abs(direction)))] + 'miss' + postfix;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		{
			var char:Character = dad;
			var animToPlay:String = singAnimations[DoubaoConfig.directionOf(Std.int(Math.abs(note.noteData)))] + note.animSuffix;
