...
class Note extends FlxSprite
{
	...
	**/
	...
	{
		// Doubao Engine: color from symmetric lane-direction table
		var colorIdx:Int = backend.DoubaoConfig.directionOf(noteData);
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false, ?createdFrom:Dynamic = null)
	{
		...
		if(noteData > -1)
		{
			...
			if(!isSustainNote) { //Doubao Engine: removed 4-lane limit so >4K lanes still animate
				...
				animToPlay = colArray[backend.DoubaoConfig.directionOf(noteData)];
		...
		if (isSustainNote && prevNote != null)
		{
			...
			animation.play(colArray[backend.DoubaoConfig.directionOf(noteData)] + 'holdend');
			...
			{
				prevNote.animation.play(colArray[backend.DoubaoConfig.directionOf(prevNote.noteData)] + 'hold');
	...
	{
		final idx:Int = backend.DoubaoConfig.directionOf(noteData); // Doubao Engine: symmetric direction color
	...
	function loadNoteAnims() {
		// Doubao Engine: color from symmetric lane-direction table
		final col:String = colArray[DoubaoConfig.directionOf(noteData)];
		if (col == null)
			return;
	...
	function loadPixelNoteAnims() {
		// Doubao Engine: color/frame from symmetric lane-direction table
		final col:String = colArray[DoubaoConfig.directionOf(noteData)];
		if (col == null)
			return;
		final px:Int = DoubaoConfig.directionOf(noteData);
